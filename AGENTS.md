...
- **Resources:** Fonts (`IndieFlower-Regular.ttf`, tahoma) and UI assets in `src/main/resources/ui/`.
...
| `UtilsJuego` | Difficulty constants (multipliers 1.0-2.0, base times 20s-4s), prize formulas. |
...
- `Categorias`, `Jugador`, `Palabra`, `Logro` (wrapper of `LogroId`), `SnapshotPartida` (inmutable, Builder for game-end state).
...
| Pluma | $45 | Reveals most frequent remaining letter |
| Marcatextos | $60 | Shows hint (disabled in Imposible) |
...
- **Premios**: $35 (Contra el Reloj) to $5,000 (Biblioteca Viviente).
- **Evaluation**:
...
- **Migrations:** `ConfigDB.initDB()` via `PRAGMA user_version`. Current version: 2.
- **Tables:** `categorias`, `palabras`, `jugadores`, `descubrimientos`, `logros (id_jugador, id_logro TEXT, PK compuesta)`.
...
- **Completed phases:**
  ...
  8. Achievements + medals (current)
- **Pending (placeholders):** `AcercaPanel`, `OpcionesPanel`.
